...
public class AsientoCaracteristicaResource implements Serializable {
    ...
    public Response findRange(
        ...
        try {
            if (firstResult >= 0 && maxResults > 0 && maxResults <= 50) {
                List<AsientoCaracteristica> encontrados = aBean.findRange(firstResult, maxResults);
                Long total = (long) aBean.count();
    ...
    public Response findById(@PathParam("id") Integer id) {
        if (id != null) {
            try {
                AsientoCaracteristica encontrado = aBean.findById(id);
                if (encontrado != null) {
                    Response.ResponseBuilder builder = Response.ok(encontrado).type(MediaType.APPLICATION_JSON);
                    return builder.build();
                }
                return Response.status(404).header("Not-Found", "Asiento con id: " + id).build();
            } catch (Exception e) {
    ...
    public Response create(AsientoCaracteristica asientoCaracteristica, @Context UriInfo uriInfo) {
        if (asientoCaracteristica != null && asientoCaracteristica.getIdAsientoCaracteristica() == null) {
            try {
                aBean.create(asientoCaracteristica);
                if (asientoCaracteristica.getIdAsientoCaracteristica() != null) {
                    UriBuilder uriBuilder = uriInfo.getAbsolutePathBuilder();
                    uriBuilder.path(String.valueOf(asientoCaracteristica.getIdAsientoCaracteristica()));
